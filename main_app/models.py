...
class FormModel(models.Model):
    user =models.ForeignKey(User, related_name='user_form', on_delete=models.CASCADE)
    category = models.CharField(max_length=100, blank=True, null=True)
    subject = models.CharField(max_length=300, blank=True, null=True)
    image=models.FileField(null=True,blank=True,upload_to="support_center/")
    is_solved = models.BooleanField(default=False)
    submitted_on = models.DateTimeField(auto_now_add=True)
    updated_on = models.DateTimeField(auto_now=True)
    
    def __str__(self):
        return f"Ticket Generated by {self.user.email} - {self.category} - {self.subject}"
    

class TicketComment(models.Model):
    user = models.ForeignKey(User, related_name = 'ticket_comment', on_delete=models.CASCADE)
    ticket = models.ForeignKey(FormModel, related_name = "ticket_model", on_delete=models.CASCADE)
    comment = models.TextField()
    create_at = models.DateTimeField(auto_now_add=True)
    
    def __str__(self):
        return f"Ticket comment by {self.user.email} at {self.ticket.category}"
